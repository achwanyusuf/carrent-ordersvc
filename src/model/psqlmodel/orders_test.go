...
func testOrdersSoftDelete(t *testing.T) {
	t.Parallel()
...
func testOrdersQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := Orders().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testOrdersSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testOrdersQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Orders().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testOrderToOneCarUsingCar(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, carDBTypes, false, carColumnsWithDefault...); err != nil {
	...
	AddCarHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *Car) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := OrderSlice{&local}
	if err = local.L.LoadCar(ctx, tx, false, (*[]*Order)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Car == nil {
...
func testOrderToOneSetOpCarUsingCar(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, carDBTypes, false, strmangle.SetComplement(carPrimaryKeyColumns, carColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, carDBTypes, false, strmangle.SetComplement(carPrimaryKeyColumns, carColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Car{&b, &c} {
		...
		if a.CarID != x.ID {
			t.Error("foreign key was wrong value", a.CarID, x.ID)
		}
	}
}
...
var (
	orderDBTypes = map[string]string{`ID`: `integer`, `CarID`: `integer`, `OrderDate`: `date`, `PickupDate`: `date`, `DropoffDate`: `date`, `PickupLocation`: `character varying`, `PickupLat`: `double precision`, `PickupLong`: `double precision`, `DropoffLocation`: `character varying`, `DropoffLat`: `double precision`, `DropoffLong`: `double precision`, `CreatedBy`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedBy`: `integer`, `UpdatedAt`: `timestamp with time zone`, `DeletedBy`: `integer`, `DeletedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
